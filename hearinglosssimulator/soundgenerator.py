...
def chirp(length, samplerate = 44100., f1 = 50, f2 = 15000, ampl = .1):
    times = np.arange(length)/samplerate
    sound = scipy.signal.chirp(times,f1,times[-1],f2)
    sound *= ampl
    return sound.astype('float32')


def crossing_chirp(length, samplerate = 44100., f1 = 50, f2 = 15000, ampl = .1):
    times = np.arange(length)/samplerate
    sound = scipy.signal.chirp(times,f1,times[-1],f2)+ scipy.signal.chirp(times,f2*.8,times[-1],f1*1.2)
    sound *= ampl
    return sound.astype('float32')

def several_sinus(length, samplerate = 44100., freqs = [ 150, 203, 498, 1120],  ampl = .1):
    times = np.arange(length)/samplerate
...
def moving_sinus(length, samplerate = 44100., speed = .5,  f1 = 900., f2 = 1000,  ampl = .1):
    times = np.arange(length)/samplerate
    f =  (np.sin(np.pi*2*speed*times)+1)/2 *  (f2-f1) + f1
    phase = np.cumsum(f/samplerate)*2*np.pi
    sound = np.sin(phase) *  ampl
    return sound.astype('float32')

def whitenoise(length, samplerate = 44100., ampl = .1):
    sound =np.random.randn(length) *  ampl
    return sound.astype('float32')

def notchnoise(length, samplerate = 44100.,  f1 = 1000., f2 = 2000., ampl = .1):
    sound =np.random.randn(length) *  ampl
    soundf = np.fft.fft(sound, n = sound.size)
    n1 =  int(f1/samplerate*sound.size)
    n2 =  int(f2/samplerate*sound.size)
    soundf[n1:n2] = 0.
    soundf[-n2:-n1] = 0.
...
def bandfiltered_noise(length, samplerate = 44100., f1 = 200., f2 = 1200., ampl = .1):
    sound = whitenoise(length, samplerate = samplerate, ampl = ampl)
    soundf = np.fft.fft(sound, n = sound.size)
    n1 =  int(f1/samplerate*sound.size)
    n2 =  int(f2/samplerate*sound.size)
    soundf[:n1] = 0.
...
def erb_noise(length, samplerate = 44100., f = 500, ampl = 0.1):
    w=24.7*(4.37e-3*f+1.)
    return bandfiltered_noise(length, samplerate = samplerate, f1 = f-w/2, f2 = f+w/2, ampl = ampl)


def moving_erb_noise(length, samplerate = 44100., f1 = 200, f2 = 1200, speed = .1,
...
def moving_mask_around_tone(length, samplerate = 44100., freqs = [ 1000.], f1 = 2000., f2 = 2500.):
